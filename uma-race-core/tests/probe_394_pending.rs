...
fn probe_394_pending() {
    ...
    let r = HorseRunner::new(
        course,
        GroundCondition::Soft,
        &horse,
        c["seed_u32"].as_u64().unwrap() as u32,
    );
    for (id, start, end) in r.debug_pending_triggers() {
        eprintln!("{id} [{start:.3}, {end:.3}]");
    }
    eprintln!("used_at_gate={:?}", r.debug_used_skills());
}
